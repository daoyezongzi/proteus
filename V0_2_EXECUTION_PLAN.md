...
## 0. 2026-08-28 配额优先的分类扫描与 1688 供应商前置过滤（已批准并实现）

实现状态：后端、只读 `1688-cli` 适配、HioBuy 兼容选择、异步进度、精简导出和
Claude/TradeEye 风格前端均已落地。当前机器已安装并登录默认 `1688-cli` profile，
已通过只读 readiness 检查；2026-08-28 已完成一次单分类受控商品搜索，具体结果和未决
问题记录在 `LOG.md`，后续运行仍按独立预算控制。
...
- 运行请求可显式设置 `enable_1688_prefilter=false` 作为临时风控降级：不调用 1688
  或执行本地登录态检查，但继续将满足本地范围/家族/需求条件的候选送入 Amazon；供应商
  阶段记为 `NOT_RUN`，最终只能是 `REVIEW_REQUIRED`，不得伪装成有供应商或商机通过。
