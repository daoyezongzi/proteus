...
如果页面已正常显示但第一页没有被识别，扩展会把有界的 DOM 探针一并写入页面证据：链接、
商品选择器命中、下一页候选、iframe/Shadow Root 和少量嵌入数据标记。页面没有商品且没有
明确空店证据时，任务会暂停为 `PARSER_FAILED`，保留探针和页号，不会直接进入自动翻页或
输出空店结论。扩展更新后请在 `edge://extensions` 点击一次 **Reload**，再重新点击工具栏
