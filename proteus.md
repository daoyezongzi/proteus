...
服务端将该探针与 `page_evidence` 一起保存；`PARSER_FAILED` 不消费已完成页号，也不生成
`EMPTY`。修复页面或扩展后可重新认领同一任务，前端会把探针摘要显示在采集状态和最近快照
中。诊断 URL 仅允许 HTTPS 1688 域名及数字页码/offer ID 参数，避免将登录/验证令牌写入
本机证据库。
