...
const sourceStatus = {
  SUCCESS: ["导入完成", "pass", "商品清单声明已完整。"],
  EMPTY: ["已证实为空", "pass", "商品清单明确报告 0 件且没有下一页。"],
  PARTIAL: ["部分导入", "review", "已保存部分商品；未导入的商品不参与淘汰。"],
  RISK_CONTROL: ["来源被阻断", "review", "原始采集遇到风控；这不是零商品。"],
  AUTH_REQUIRED: ["来源需登录", "review", "原始采集需要用户完成登录。"],
  TIMEOUT: ["来源超时", "error", "原始采集未在等待窗口内完成响应。"],
  PARSER_FAILED: ["来源无法确认", "error", "原始采集没有足够结构化证据。"],
  CLI_ERROR: ["来源采集失败", "error", "外部采集器没有取得可用商品证据。"],
  NOT_CONFIGURED: ["等待导入", "review", "请先导入用户或 Agent 生成的商品 JSON。"],
};
...
function setBusy(busy) {
  ...
  button.disabled = busy || importBusy || !$("#supplier_id").value || !activeSnapshotId || selectedCategories().length === 0;
  $(".button__label", button).textContent = busy ? "筛选中…" : "使用快照筛选";
  button.dataset.state = busy ? "busy" : "idle";
  const importButton = $("#importInventoryButton");
  if (importButton) importButton.disabled = busy || importBusy || !$("#supplier_id").value;
}
...
function showSourceVerdict(outcome) {
  ...
  element.innerHTML = `<strong>${esc(label)}</strong>${esc(description)} <span>${observed} 件已导入 / ${esc(available)} 件报告总量；${Number(outcome.pages_completed || 0)} 页完成。</span>`;
}
...
async function saveSupplier() {
  ...
  try {
    ...
    setStatus(`已保存「${saved.label}」；现在可以导入商品 JSON。`, "success");
  } catch (error) {
...
function renderSnapshotStatus(snapshot = null) {
  ...
  if (!snapshot) {
    $("#sourceVerdict").hidden = true;
    element.dataset.state = "idle";
    element.innerHTML = "<strong>尚未导入商品清单</strong><span>选择已保存供应商后，上传用户或 Agent 生成的 JSON 文件。</span>";
    return;
  }
  const status = snapshot.acquisition_status || "UNKNOWN";
  const usable = snapshotCanRun(snapshot);
  const source = snapshot.source_method === "AGENT_JSON_IMPORT" ? "JSON 导入" : "历史快照";
  element.dataset.state = usable ? "ready" : "error";
  const coverage = `${Number(snapshot.pages_completed || 0)} 页、${Number(snapshot.observed_offer_count || 0)} 件`;
  const completeness = snapshot.inventory_complete ? "声明已完整" : "声明为部分清单";
  element.innerHTML = `<strong>${esc(source)} · ${esc(sourceStatus[status]?.[0] || status)}</strong><span>${coverage}；${completeness}。${usable ? "可以开始市场分析或再次导入。" : "当前快照不能用于分析，请导入包含有效商品的 JSON。"}</span>`;
}

function snapshotCanRun(snapshot) {
  if (!snapshot) return false;
  const observed = Number(snapshot.observed_offer_count || 0);
  const complete = snapshot.inventory_complete === true;
  return (snapshot.acquisition_status === "SUCCESS" && complete && observed > 0)
    || (snapshot.acquisition_status === "EMPTY" && complete && observed === 0)
    || (snapshot.acquisition_status === "PARTIAL" && observed > 0);
}
...
async function loadLatestSnapshot() {
  ...
  try {
    ...
    else renderSnapshotStatus(null);
  } catch (error) {
    $("#sourceVerdict").hidden = true;
    renderSnapshotStatus({ acquisition_status: "ERROR", pages_completed: 0, observed_offer_count: 0 });
...
async function importInventory() {
  ...
  if (!file) {
    setStatus("请选择用户或 Agent 生成的商品 JSON 文件。", "error");
    return;
  }
  const button = $("#importInventoryButton");
  importBusy = true;
  button.disabled = true;
  syncRunAvailability();
  try {
    ...
    if (new TextEncoder().encode(text).length > 10 * 1024 * 1024) {
      throw new Error("JSON 文件超过 10 MB 上限。请让 Agent 分批生成或压缩字段。");
    }
    let document;
    try {
      document = JSON.parse(text);
    } catch (_error) {
      throw new Error("文件不是有效 JSON，请检查编码和语法。");
    }
    if (!document || Array.isArray(document) || typeof document !== "object") {
      throw new Error("JSON 顶层必须是对象。");
    }
    const imported = await json(`/supplier-scout/suppliers/${encodeURIComponent(supplier.supplier_id)}/snapshots/import`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        filename: file.name,
    ...
    if (imported.snapshot) useSnapshot(imported.snapshot);
    await loadLatestSnapshot();
    const summary = imported.import || {};
    setStatus(`已导入 ${summary.valid_offer_count || 0} 件商品；${summary.invalid_offer_count || 0} 条无效记录、${summary.duplicate_offer_count || 0} 条重复记录。`, imported.can_run ? "success" : "error");
  } catch (error) {
...
function renderSummary(result) {
  ...
  $("#runSummary").innerHTML = [
    summaryCell("清单覆盖", `${formatNumber(inventory.observed_offer_count, 0)} / ${available}`, inventory.inventory_complete ? "导入清单声明完整" : "仅覆盖导入的部分商品"),
  ...
  if (result.status === "SOURCE_BLOCKED") {
    ...
    notice.hidden = false;
    notice.dataset.tone = "review";
    notice.textContent = `当前仅导入 ${inventory.observed_offer_count || 0} 件；清单声明不完整，未导入部分不参与淘汰。`;
  } else {
    notice.hidden = true;
  }
}
...
async function submitRun(event) {
  ...
  if (!request.supplier_id || !request.inventory_snapshot_id || request.selected_category_ids.length === 0) {
    setStatus("请选择供应商、先导入有效商品 JSON，并至少保留一个 ACTIVE 小类。", "error");
    return;
  }
  ...
  setBusy(true);
  setStatus("正在使用已封存的 JSON 商品快照开始筛选…");
  $("#emptyState").hidden = true;
  $("#resultContent").hidden = false;
  $("#runNotice").hidden = false;
  $("#runNotice").textContent = "正在排队复用商品快照…";
  try {
    ...
    renderResult(result);
    setStatus("商品快照与筛选结果已封存；请先看清单覆盖状态，再看 A / A-。", "success");
  } catch (error) {
    $("#runNotice").hidden = false;
...
$("#importInventoryButton").addEventListener("click", importInventory);
